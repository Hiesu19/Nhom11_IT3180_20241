...
function InvoicePreview() {
  ...
  const momoQRCodeURL = `https://payment.momo.vn?phone=${momoPhoneNumber}&amount=${total}&message=${encodeURIComponent(paymentMessage)}`;
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex items-center justify-center p-5">
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg p-6">
        <h1 className="text-2xl font-bold mb-6 text-center">Thông tin hóa đơn</h1>
        ...
        <div className="flex justify-between mt-4 space-x-4">
          <button
            ...
            className={`w-1/2 px-4 py-2 rounded ${paymentMethod === 'cash' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
          >
            Thanh toán Tiền mặt
          </button>
          <button
            onClick={() => handlePaymentMethodChange('banking')}
            className={`w-1/2 px-4 py-2 rounded ${paymentMethod === 'banking' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
          >
        ...
        {paymentMethod === 'banking' && (
          <p className="text-center mt-4">Quét mã QR bằng ứng dụng Momo để thanh toán.</p>
        )}
